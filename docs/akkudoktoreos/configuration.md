...
Use endpoint `GET /v1/config/file` with parameter `update=True`to update the configuration
from the `EOS configuration file`.
...
Settings are sets of configuration data. Settings have higher priority to all other configuration
data from all other sources. Settings are not persistent. To make a current configuration with the
current settings persistent save the configuration to the EOS configuration file.

Use the `PUT /v1/config` endpoint to change the current configuration settings.
