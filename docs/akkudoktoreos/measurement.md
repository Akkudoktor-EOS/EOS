...
The state of charge (SoC) of batteries and electric vehicle batteries can be stored.
...
You should make the `measurement key`s start with `<device_id>` to easily associate the soc with a
battery or electric vehicle device (e.g. `bat1_soc`).
