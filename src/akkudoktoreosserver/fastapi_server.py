...
def fastapi_gesamtlast(
    ...
    if measured_data_df["time"].dt.tz is None:
        measured_data_df["time"] = measured_data_df["time"].dt.tz_localize(
            "Europe/Berlin"
        )
    else:
        measured_data_df["time"] = measured_data_df["time"].dt.tz_convert(
            "Europe/Berlin"
        )
...
@app.post("/optimize")
def fastapi_optimize(parameters: Annotated[OptimizationParameters, Query()]):
    from datetime import datetime

    # Perform optimization simulation
    result = opt_class.optimierung_ems(
        parameters=parameters, start_hour=datetime.now().hour
    )
    print(result)
...
if __name__ == "__main__":
    ...
    try:
        uvicorn.run(app, host=host, port=port)  # Run the FastAPI application
