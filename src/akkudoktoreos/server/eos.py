...
def cache_cleanup_task() -> None:
    """Task to clear cache from expired cache files."""
...
async def energy_management_task() -> None:
    """Task for energy management."""
    logger.debug("Check EMS run")
    await get_ems().manage_energy()
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    if config_eos.cache.cleanup_interval is None:
        ...
        cache_cleanup_repeated_task = make_repeated_task(
            cache_cleanup_task,
            seconds=float(config_eos.cache.cleanup_interval),
            on_exception=cache_cleanup_on_exception,
        )
        asyncio.create_task(cache_cleanup_repeated_task())

    energy_management_repeated_task = make_repeated_task(
        energy_management_task,
        seconds=10,
        wait_first=config_eos.ems.startup_delay,
        on_exception=energy_management_on_exception,
    )
    asyncio.create_task(energy_management_repeated_task())
