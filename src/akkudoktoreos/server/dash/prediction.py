...
def PVForecast(predictions: pd.DataFrame, config: dict) -> FT:
...
def ElectricityPriceForecast(predictions: pd.DataFrame, config: dict) -> FT:
    source = ColumnDataSource(predictions)
...
def WeatherTempAirHumidityForecast(predictions: pd.DataFrame, config: dict) -> FT:
    source = ColumnDataSource(predictions)
...
def WeatherIrradianceForecast(predictions: pd.DataFrame, config: dict) -> FT:
    source = ColumnDataSource(predictions)
    provider = config["weather"]["provider"]
...
def LoadForecast(predictions: pd.DataFrame, config: dict) -> FT:
    source = ColumnDataSource(predictions)
...
def Prediction(eos_host: str, eos_port: Union[str, int]) -> str:
    ...
    return Grid(
        PVForecast(predictions, config),
        ElectricityPriceForecast(predictions, config),
        WeatherTempAirHumidityForecast(predictions, config),
        WeatherIrradianceForecast(predictions, config),
        LoadForecast(predictions, config),
        cols_max=2,
    )
