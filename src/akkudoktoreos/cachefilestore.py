...
class CacheFileStore(metaclass=CacheFileStoreMeta):
    ...
    def _generate_cache_file_key(
        ...
        key_datetime = to_datetime(until_datetime, as_string="%Y-%m-%dT%H:%M:%S")
    ...
    def _search(
        ...
        for cache_file_key, cache_item in self._store.items():
            cache_file_datetime = cache_item[
                1
    ...
    def create(
        ...
        if until_datetime:
            ...
            until_datetime = datetime.now() + with_ttl
        elif until_date:
    ...
    def delete(
        ...
        if until_datetime:
            ...
            before_datetime = datetime.combine(tomorrow, time.min)
...
def cache_in_file(
    ...
    'until_datetime` or `with_ttl` parameter. The value of this parameter will be used instead of
    the one given in the decorator if available.
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            for param in ["until_datetime", "with_ttl", "until_date"]:
                if param in kwargs:
