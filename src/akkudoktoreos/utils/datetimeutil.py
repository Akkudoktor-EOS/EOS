...
class Time(pendulum.Time):
    ...
    def _serialize(cls, value: Optional["Time"]) -> str:
        ...
        if value.tzinfo is not None:
            # Use a more standard ISO format for timezone-aware times
            return value.format("HH:mm:ss.SSSSSSZ")
        else:
...
def _parse_time_string(time_str: str, default_date: pendulum.Date = None) -> pendulum.Time:
    ...
    tz_pattern = re.compile(
        r"(.+?)\s*([+-]\d{2}:?\d{2}|UTC|GMT|EST|CST|MST|PST|EDT|CDT|MDT|PDT|[A-Z]{3,4})$",
        re.IGNORECASE,
    )
    ...
    if tz_match:
        ...
        if timezone_str.upper() in ["UTC", "GMT"]:
            ...
            if len(clean_tz) == 5:  # +HHMM or -HHMM
                ...
                timezone_info = (
                    pendulum.datetime(2000, 1, 1, tz="UTC").add(minutes=offset_minutes).tzinfo
                )
        else:
...
class TimeWindow(BaseModel):
    ...
    def _window_start_end(self, reference_date: DateTime) -> tuple[DateTime, DateTime]:
        """Get the actual start and end datetime for the time window on a given date.

        Args:
            reference_date: The date to calculate the window for.

        Returns:
            Tuple of (start_datetime, end_datetime) for the window.
        """
