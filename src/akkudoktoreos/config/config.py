...
class ConfigEOS(SingletonMixin, SettingsEOSDefaults):
    ...
    def settings_customise_sources(
        ...
        def lazy_data_folder_path_settings() -> dict:
            ...
            # 1. From EOS_DATA_DIR
            if env_dir := os.getenv(cls.EOS_DATA_DIR):
                try:
                    data_dir = Path(env_dir).resolve()
                    data_dir.mkdir(parents=True, exist_ok=True)
                    data_folder_settings["general"]["data_folder_path"] = data_dir
                    return data_folder_settings
                except Exception as e:
                    logger.warning(f"Could not setup data dir {data_dir}: {e}")

            # 2. From EOS_DIR env
            if env_dir := os.getenv(cls.EOS_DIR):
                try:
                    data_dir = Path(env_dir).resolve()
                    data_dir.mkdir(parents=True, exist_ok=True)
                    data_folder_settings["general"]["data_folder_path"] = data_dir
            ...
            try:
                data_dir = Path(user_data_dir(cls.APP_NAME, cls.APP_AUTHOR))
