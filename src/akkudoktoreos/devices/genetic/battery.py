...
class Battery:
    ...
    def __init__(
        self, parameters: BaseBatteryParameters, optimization_hours: int, prediction_hours: int
    ):
    ...
    def _setup(self) -> None:
        ...
        self.discharge_array = np.full(self.optimization_hours, 1)
        self.charge_array = np.full(self.optimization_hours, 1)
    ...
    def set_discharge_per_hour(self, discharge_array: np.ndarray) -> None:
        ...
        if len(discharge_array) != self.prediction_hours:
            raise ValueError(f"Discharge array must have exactly {self.prediction_hours} elements.")
    ...
    def set_charge_per_hour(self, charge_array: np.ndarray) -> None:
        ...
        if len(charge_array) != self.prediction_hours:
            raise ValueError(f"Charge array must have exactly {self.prediction_hours} elements.")
