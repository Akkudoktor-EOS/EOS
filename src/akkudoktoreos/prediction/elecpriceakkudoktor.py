...
class AkkudoktorElecPrice(PydanticBaseModel):
    ...
    values: List[List[AkkudoktorElecPriceValue]]
...
class ElecPriceAkkudoktor(ElecPriceProvider):
    ...
    def _update_data(self, force_update: Optional[bool] = False) -> None:
        ...
        for i in range(values_len):
            ...
            original_datetime = akkudoktor_data.values[0][i].start
            # print(original_datetime," ",sum_dc_power," ",akkudoktor_data.values[0][i].dcPower)
            ...
            record = ElecPriceDataRecord(
                ...
                elecprice_marketprice=akkudoktor_data.values[0][i].marketpriceEurocentPerKWh,
            )
            self.append(record)
