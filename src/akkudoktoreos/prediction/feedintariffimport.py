...
class FeedInTariffImport(FeedInTariffProvider, PredictionImportProvider):
    ...
    def _update_data(self, force_update: Optional[bool] = False) -> None:
        if self.config.feedintariff.provider_settings is None:
            logger.debug(f"{self.provider_id()} data update without provider settings.")
            return
        if self.config.feedintariff.provider_settings.import_file_path:
            self.import_from_file(
                self.config.provider_settings.import_file_path, key_prefix="feedintariff"
            )
        if self.config.feedintariff.provider_settings.import_json:
            self.import_from_json(
                self.config.feedintariff.provider_settings.import_json, key_prefix="feedintariff"
            )
