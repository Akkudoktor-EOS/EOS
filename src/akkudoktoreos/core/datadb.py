...
class DataDB(ABC, SingletonMixin, ConfigMixin):
...
class SQLiteDatabase(DataDB):
    ...
    def save_record(self, key: bytes, value: bytes, namespace: Optional[str] = None) -> None:
        ...
        if not isinstance(self.conn, sqlite3.Connection):
            raise ValueError(f"SQLite3 connection is of wrong tpe `{type(self.conn)}`.")
...
database_eos: Optional[DataDB] = None


def set_database(database: Optional[DataDB]) -> None:
    global database_eos

    if database_eos:
        database_eos.close()
    database_eos = database
    if database_eos:
        database_eos.open()


def database_track_config(config_eos: Any, path: str, old_value: Any, value: Any) -> None:
    """Track database config changes."""
    global database_eos

    if not path.startswith("database"):
        raise ValueError(f"Database shall not track '{path}'")

    if path == "database/provider":
        if (value != old_value) or (value and database_eos is None):
            database: Optional[DataDB] = None
            if value == "LMDB":
...
def get_database() -> Optional[DataDB]:
    """Get the EOS database."""
    global database_eos
    ...
    if not database_eos:
        ...
        database_track_config(config_eos, "database/provider", None, config_eos.database.provider)
