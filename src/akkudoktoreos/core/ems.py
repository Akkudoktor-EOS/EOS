...
class EnergieManagementSystem(SingletonMixin, ConfigMixin, PredictionMixin, PydanticBaseModel):
    start_datetime: Optional[DateTime] = Field(
        default=None,
        description="The starting datetime of the current or latest energy management.",
    )

    @field_validator("start_datetime", mode="after")
    @classmethod
    def validate_start_datetime(cls, value: Optional[DateTime]) -> DateTime:
        start_datetime = value
        if start_datetime is None:
            start_datetime = to_datetime()
        start_datetime = start_datetime.set(minute=0, second=0, microsecond=0)
        logger.debug(f"Start datetime: '{start_datetime}' from '{value}'")
    ...
    def set_akku_discharge_hours(self, ds: List[int]) -> None:
        if self.akku is not None:
            self.akku.set_discharge_per_hour(np.array(ds, int))
    ...
    def set_ev_charge_hours(self, ds: List[float]) -> None:
    ...
    def set_start_hour(self, start_hour: Optional[int] = None) -> None:
        ...
        if start_hour is None:
            self.start_datetime = to_datetime().set(minute=0, second=0, microsecond=0)
        else:
            self.start_datetime = to_datetime().set(
...
def get_ems() -> EnergieManagementSystem:
    """Gets the EOS Enery Managemnt System."""
    return ems
