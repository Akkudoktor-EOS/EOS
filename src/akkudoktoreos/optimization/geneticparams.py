...
from akkudoktoreos.core.coreabc import (  # EnergyManagementSystemMixin, # Creates circular dependency with ems.py; StartMixin,                  # Creates circular dependency with ems.py
    ConfigMixin,
    MeasurementMixin,
    PredictionMixin,
)
...
class EnergyManagementParameters(ParametersBaseModel):
    """Encapsulates energy-related forecasts and costs used in optimization."""
...
class OptimizationParameters(
    ...
    @classmethod
    def prepare(cls) -> "Optional[OptimizationParameters]":
        ...
        Returns:
            OptimizationParameters: The fully prepared optimization input object.
        ...
        oparams: "Optional[OptimizationParameters]" = None
        ...
        end_datetime = ems.start_datetime.add(hours=cls.config.optimization.hours)
        max_retries = 10
        ...
        for attempt in range(1, max_retries + 1):
            ...
            try:
                battery = cls.config.devices.batteries[0]
            except:
                logger.exception(
                ...
                # Retry
                continue
            if battery.levelized_cost_of_storage_kwh is None:
                logger.error(
                    "No battery device LCOS data available - defaulting to 0 €/kWh. Parameter preparation attempt {}.",
                    attempt,
                )
                battery.levelized_cost_of_storage_kwh = 0
            battery_lcos_kwh = battery.levelized_cost_of_storage_kwh
            ...
            try:
                electric_vehicle = cls.config.devices.electric_vehicles[0]
            except:
                logger.exception(
            ...
            try:
                inverter = cls.config.devices.inverters[0]
            except:
                logger.exception(
            ...
            try:
                oparams = OptimizationParameters(
                    ems=EnergyManagementParameters(
