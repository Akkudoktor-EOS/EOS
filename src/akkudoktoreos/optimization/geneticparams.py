    ...
    def prepare(cls) -> "Optional[GeneticOptimizationParameters]":
        ...
        for attempt in range(1, max_retries + 1):
            ...
            if cls.config.devices.max_batteries == 0:
                ...
                try:
                    initial_soc = cls.measurement.key_to_value(
                        key=battery_config.measurement_key_soc,
                ...
                if initial_soc is None:
                    logger.error(
                        f"No battery device SoC data (measurement key = '{battery_config.measurement_key_soc}') available - defaulting to 0%."
                    )
                    initial_soc = 0
            ...
            if cls.config.devices.max_electric_vehicles == 0:
                ...
                try:
                    initial_soc = cls.measurement.key_to_value(
                        key=electric_vehicle_config.measurement_key_soc,
                        target_datetime=ems.start_datetime,
                    )
                except:
                    initial_soc = None
                if initial_soc is None:
                    logger.error(
                        f"No electric vehicle device SoC data (measurement key = '{electric_vehicle_config.measurement_key_soc}') available - defaulting to 0%."
                    )
                    initial_soc = 0
