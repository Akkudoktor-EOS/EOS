...
class GeneticOptimizationParameters(
    ...
    def prepare(cls) -> "Optional[GeneticOptimizationParameters]":
        ...
        if cls.config.optimization.hours is None:
            logger.error("Optimization hours unknown - defaulting to 24 hours.")
            cls.config.optimization.hours = 24
        ...
        for attempt in range(1, max_retries + 1):
            ...
            if cls.config.devices.max_batteries == 0:
                ...
                if (
                    cls.config.measurement.battery_soc_keys is None
                    or len(cls.config.measurement.battery_soc_keys) == 0
                ):
                    initial_soc = None
                else:
                    key = next(
                        (
                            k
                            for k in cls.config.measurement.battery_soc_keys
                            if k.startswith(battery_config.device_id)
                        ),
                        None,
                    )
                    if key is None:
                        initial_soc = cls.measurement.key_to_value(
                            key=cls.config.measurement.battery_soc_keys[0],
                            target_datetime=ems.start_datetime,
                        )
                    else:
                        initial_soc = cls.measurement.key_to_value(key, ems.start_datetime)
                if initial_soc is None:
                    logger.error("No battery device SoC data available - defaulting to 0%.")
                    initial_soc = 0
                battery_params.initial_soc_percentage = initial_soc
            ...
            if cls.config.devices.max_electric_vehicles == 0:
                ...
                try:
                    ...
                    continue
                if (
                    cls.config.measurement.electric_vehicle_soc_keys is None
                    or len(cls.config.measurement.electric_vehicle_soc_keys) == 0
                ):
                    initial_soc = None
                else:
                    key = next(
                        (
                            k
                            for k in cls.config.measurement.electric_vehicle_soc_keys
                            if k.startswith(battery_config.device_id)
                        ),
                        None,
                    )
                    if key is None:
                        initial_soc = cls.measurement.key_to_value(
                            key=cls.config.measurement.electric_vehicle_soc_keys[0],
                            target_datetime=ems.start_datetime,
                        )
                    else:
                        initial_soc = cls.measurement.key_to_value(key, ems.start_datetime)
                if initial_soc is None:
                    logger.error(
                        "No electric vehicle device SoC data available - defaulting to 0%."
                    )
                    initial_soc = 0
                electric_vehicle_params.initial_soc_percentage = initial_soc
