...
from akkudoktoreos.devices.genetic.generic import HomeAppliance
...
from akkudoktoreos.optimization.geneticsolution import (
    OptimizeResponse,
    SimulationResult,
)
...
class GeneticOptimization(OptimizationBase):
    ...
    def __init__(
        ...
        if self.config.devices.max_electric_vehicles < 1:
            error_msg = f"No EV configuration. max_electric_vehicles = {self.config.devices.max_electric_vehicles}\n{self.config}"
            logger.error(error_msg)
            raise ValueError(error_msg)
        try:
            ...
            raise ValueError(error_msg)
        self.ev_possible_charge_values: list[float] = charge_rates
    ...
    def optimierung_ems(
        ...
        if parameters.pv_akku:
            akku = Battery(
                parameters.pv_akku,
                optimization_hours=self.config.optimization.hours,
                prediction_hours=self.config.prediction.hours,
            )
            # self.devices.add_device(akku)
            akku.set_charge_per_hour(np.full(self.config.optimization.hours, 1))
        ...
        if parameters.eauto:
            ...
            eauto.set_charge_per_hour(np.full(self.config.optimization.hours, 1))
            self.optimize_ev = (
        ...
        self.simulation.set_start_hour(start_hour)
