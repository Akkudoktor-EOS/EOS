...
class TestElecPriceProvider:
    ...
    def test_singleton_instance(self, provider):
        ...
        another_instance = _ElecPriceProviderForTest()
...
class TestElecPriceProviderApplyFeesValidation:
    """Tests for input validation in ElecPriceProvider.apply_fees()."""
...
class TestElecPriceProviderApplyFees:
    """Tests for ElecPriceProvider.apply_fees(), with keys_to_dataframe() mocked."""
    ...
    async def test_apply_fees_missing_fee_rows_filled_with_zero(self, provider, monkeypatch):
        ...
        df_elecfee = pd.DataFrame(
            {
                "elecfee_consumption_amt_wh": [0.000288, 0.000288],
                "elecfee_consumption_percent_amt": [19.0, 19.0],
            },
            index=idx_partial,
        )
        _patch_keys_to_dataframe(monkeypatch, provider, df_elecfee)
