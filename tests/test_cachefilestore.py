...
def test_generate_cache_file_key(cache_store):
    ...
    valid_until = to_datetime("2024-10-01").date()
    cache_file_key, cache_file_valid_until = cache_store._generate_cache_file_key(key, valid_until)
    ...
    assert cache_file_valid_until == valid_until
...
def test_clear_cache_files_by_date(cache_store):
    ...
    cache_store.clear(clear_until=datetime.now())
    ...
    cache_store.clear(clear_until=datetime.now() + timedelta(days=1))
...
def test_cache_file_with_date(cache_store):
    ...
    cache_file = cache_store.create("dated_file", mode="w+", valid_until=specific_date)
    ...
    retrieved_file = cache_store.get("dated_file", valid_until=specific_date)
    assert retrieved_file is not None
    retrieved_file.seek(0)
