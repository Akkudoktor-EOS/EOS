...
class DummyInstruction(BaseInstruction):
    """A dummy instruction for testing with a fixed duration."""

    def __init__(self, resource_id, execution_time, duration_seconds: Optional[int]):
        super().__init__(resource_id=resource_id, execution_time=execution_time, abnormal_condition=False)
        self._duration_seconds = duration_seconds

    def duration(self) -> Optional[Duration]:
        if self._duration_seconds is None:
            return None
        return to_duration(self._duration_seconds)
...
class TestEnergyManagementPlan:
    def test_add_instruction_and_time_range(self, fixed_now):
        ...
        instr1 = DummyInstruction(
            resource_id="dev-1",
            execution_time=fixed_now,
            duration_seconds=600
        )
        instr2 = DummyInstruction(
            resource_id="dev-2",
