...
class TestParseTimeString:
    """Test suite for the _parse_time_string function."""

    def test_parse_basic_24_hour_formats(self):
        """Test parsing basic 24-hour formats."""
        test_cases = [
            ("14:30", 14, 30, 0, 0),
            ("14:30:45", 14, 30, 45, 0),
        ...
        for time_str, expected_hour, expected_minute, expected_second, expected_microsecond in test_cases:
            result = _parse_time_string(time_str)
    ...
    def test_parse_12_hour_formats(self):
        """Test parsing 12-hour AM/PM formats."""
        test_cases = [
            ("2:30 PM", 14, 30, 0),
            ("2:30:45 PM", 14, 30, 45),
            ("11:30 AM", 11, 30, 0),
            ("12:30 AM", 0, 30, 0),  # 12:30 AM is 00:30
            ("12:30 PM", 12, 30, 0),  # 12:30 PM is 12:30
            ("1:00 AM", 1, 0, 0),
            ("1:00 PM", 13, 0, 0),
        ]
    ...
    def test_parse_short_ampm_formats(self):
        """Test parsing short AM/PM formats."""
        test_cases = [
            ("2PM", 14, 0, 0),
            ("2AM", 2, 0, 0),
            ("11AM", 11, 0, 0),
    ...
    def test_parse_european_format(self):
        """Test parsing European format with 'h'."""
        test_cases = [
            ("14h30", 14, 30, 0),
            ("9h15", 9, 15, 0),
            ("0h00", 0, 0, 0),
    ...
    def test_parse_decimal_time(self):
        ...
        test_cases = [
            ("14.5", 14, 30, 0),  # 14.5 hours = 14:30
            ("9.25", 9, 15, 0),   # 9.25 hours = 9:15
            ("0.5", 0, 30, 0),    # 0.5 hours = 0:30
        ]
    ...
    def test_parse_with_timezone_abbreviations(self):
        """Test parsing times with timezone abbreviations."""
        test_cases = [
            ("14:30 UTC", 14, 30, "UTC"),
            ("14:30 GMT", 14, 30, "UTC"),  # GMT maps to UTC
            ("2:30 PM EST", 14, 30, "America/New_York"),
            ("9:30 PST", 9, 30, "America/Los_Angeles"),
        ]

        for time_str, expected_hour, expected_minute, expected_tz in test_cases:
            result = _parse_time_string(time_str)
            assert result.hour == expected_hour
            assert result.minute == expected_minute
            if expected_tz == "UTC":
                assert result.tzinfo == pendulum.timezone("UTC")
            else:
                assert result.tzinfo == pendulum.timezone(expected_tz)

    def test_parse_with_timezone_offsets(self):
        """Test parsing times with timezone offsets."""
        test_cases = [
            ("14:30 +05:30", 14, 30),
            ("14:30 -08:00", 14, 30),
            ("14:30 +0530", 14, 30),
    ...
    def test_parse_alternative_separators(self):
        """Test parsing with alternative separators."""
        test_cases = [
            ("14-30", 14, 30, 0),
            ("14 30", 14, 30, 0),
            ("14-30-45", 14, 30, 45),
    ...
    def test_parse_invalid_formats(self):
        ...
        invalid_cases = [
            ...
            "invalid",  # Non-numeric
            "14:30:45:12:34",  # Too many parts
            "13:30 XM",  # Invalid AM/PM
            "25PM",  # Invalid 12-hour format
        ]

        for invalid_str in invalid_cases:
            with pytest.raises(ValueError):
                _parse_time_string(invalid_str)
...
class TestTimeUtilityIntegration:
    ...
    def test_time_roundtrip_serialization(self):
        ...
        assert serialized == "14:30:45.123456+00:00"
