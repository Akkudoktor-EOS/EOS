...
@pytest.fixture(params=[LMDBDatabase, SQLiteDatabase])
def database_class(request) -> Type[DataDB]:
    """Parametrized fixture to test all database backends."""
    return request.param


@pytest.fixture
def database_instance(config_eos, database_class: Type[DataDB], temp_dir: Path) -> Iterator[DataDB]:
    """Create a database instance for testing."""
    config_eos.database.compression_level = 6
    db = database_class()
    # register as global test singleton (signature: set_database(Optional[DataDB]))
    set_database(db)
...
class TestDataDB:
    ...
    def test_database_creation(self, config_eos, database_class):
        """Test database creation and initialization."""
        config_eos.database.compression_level = 6
        db = database_class()

        assert db.storage_path == config_eos.general.data_folder_path / "db" / db.__class__.__name__.lower()
    ...
    def test_database_open_close(self, database_instance):
        ...
        assert database_instance.connection is not None

        database_instance.close()
        assert database_instance.is_open is False
    ...
    def test_compression(self, config_eos, database_class, temp_dir):
        ...
        db_compressed = database_class()
        set_database(db_compressed)
        serialized_compressed = db_compressed.serialize_data(large_data)
        deserialized_compressed = db_compressed.deserialize_data(serialized_compressed)

        # Database without compression
        config_eos.database.compression_level = 0
        db_uncompressed = database_class()

        set_database(db_uncompressed) # closes db_compressed
        serialized_uncompressed = db_uncompressed.serialize_data(large_data)
        deserialized_uncompressed = db_uncompressed.deserialize_data(serialized_uncompressed)
        ...
        set_database(None) # closes db_uncompressed
