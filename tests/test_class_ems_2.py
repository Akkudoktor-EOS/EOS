...
def test_simulation(create_ems_instance):
    ...
    assert len(result["battery_soc_per_hour"]) == 48, (
        "The length of 'battery_soc_per_hour' should be 48."
    )

    # Verfify DC and AC Charge Bins
    assert abs(result["battery_soc_per_hour"][2] - 44.70681818181818) < 1e-5, (
        "'battery_soc_per_hour[2]' should be 44.70681818181818."
    )
    assert abs(result["battery_soc_per_hour"][10] - 10.0) < 1e-5, (
    ...
    assert abs(result["battery_soc_per_hour"][20] - 10) < 1e-5, (
