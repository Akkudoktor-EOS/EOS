...
from akkudoktoreos.class_inverter_old import Wechselrichter as Inverter
...
def inverter(mock_battery):
    return Inverter(max_leistung_wh=500.0, akku=mock_battery)


def test_energie_verarbeiten_excess_generation(inverter, mock_battery):
    ...
    grid_feed_in, grid_draw, losses, self_consumption = inverter.energie_verarbeiten(
...
def test_energie_verarbeiten_generation_equals_consumption(inverter, mock_battery):
    generation = 300.0
    consumption = 300.0
    hour = 12
...
def test_energie_verarbeiten_battery_discharges(inverter, mock_battery):
...
def test_energie_verarbeiten_battery_empty(inverter, mock_battery):
...
def test_energie_verarbeiten_battery_full_at_start(inverter, mock_battery):
...
def test_energie_verarbeiten_insufficient_generation_no_battery(inverter, mock_battery):
...
def test_energie_verarbeiten_insufficient_generation_battery_assists(inverter, mock_battery):
...
def test_energie_verarbeiten_zero_generation(inverter, mock_battery):
...
def test_energie_verarbeiten_zero_consumption(inverter, mock_battery):
    ...
    assert grid_feed_in == pytest.approx(390.0, rel=1e-2)  # Excess energy after battery charges
    assert grid_draw == 0.0  # No grid draw as no consumption
...
def test_energie_verarbeiten_zero_generation_zero_consumption(inverter, mock_battery):
    generation = 0.0
    consumption = 0.0
    hour = 12
    ...
    assert grid_feed_in == 0.0  # No feed-in
    assert grid_draw == 0.0  # No grid draw
    assert losses == 0.0  # No losses
    assert self_consumption == 0.0  # No consumption


def test_energie_verarbeiten_partial_battery_discharge(inverter, mock_battery):
    ...
    assert grid_feed_in == 0.0  # No feed-in due to insufficient generation
    assert grid_draw == pytest.approx(
        150.0, rel=1e-2
...
def test_energie_verarbeiten_consumption_exceeds_max_no_battery(inverter, mock_battery):
    ...
    assert grid_feed_in == 0.0  # No feed-in
    assert grid_draw == pytest.approx(900.0, rel=1e-2)  # Grid covers the remaining shortfall
...
def test_energie_verarbeiten_zero_generation_full_battery_high_consumption(inverter, mock_battery):
    ...
    assert grid_feed_in == 0.0  # No feed-in due to zero generation
    assert grid_draw == pytest.approx(
        100.0, rel=1e-2
