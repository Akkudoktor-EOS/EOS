...
def test_branch(rebase_branch, integration_branch):
    ...
    # Test the rebased branch
    if not run_command([".venv/bin/pytest", "-vs", "--cov", "src", "--cov-report", "term-missing"]):
...
def integrate_prs(pr_numbers, integration_branch):
    ...
    if remembered_prs and remembered_prs != pr_numbers:
        ...
        reset_integration_branch(integration_branch)
    ...
    if last_step:
        logging.info(f"Resuming from step {last_step} for PR #{last_pr}")

    # Create or checkout the integration branch
    create_integration_branch_if_needed(integration_branch)
